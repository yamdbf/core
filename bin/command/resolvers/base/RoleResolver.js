...
class RoleResolver extends Resolver_1.Resolver {
    ...
    async resolve(message, command, name, value) {
        ...
        if (roleRegex.test(value)) {
            ...
            if (!role)
                throw new Error(res(BaseStrings_1.BaseStrings.RESOLVE_ERR_RESOLVE_TYPE_ID, { name, arg: value, usage, type: 'Role' }));
        }
        else {
            ...
            role = roles.first();
            if (!role)
                throw new Error(res(BaseStrings_1.BaseStrings.RESOLVE_ERR_RESOLVE_TYPE_TEXT, { name, arg: value, usage, type: 'Role' }));
        }
        return role;
    }
}
