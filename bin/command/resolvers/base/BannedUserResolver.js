...
class BannedUserResolver extends Resolver_1.Resolver {
    ...
    async resolve(message, command, name, value) {
        ...
        if (idRegex.test(value)) {
            ...
            if (!user)
                throw new Error(res(BaseStrings_1.BaseStrings.RESOLVE_ERR_RESOLVE_TYPE_ID, { name, arg: value, usage, type: 'BannedUser' }));
        }
        else {
            ...
            user = users.first();
            if (!user)
                throw new Error(res(BaseStrings_1.BaseStrings.RESOLVE_ERR_RESOLVE_TYPE_TEXT, { name, arg: value, usage, type: 'BannedUser' }));
        }
        return user;
    }
}
