...
export class Time
{
	...
	{
		const difference: Partial<Difference> = {};
		let ms: number = a - b;
		difference.raw = ms;

		const days: number = Math.floor(ms / 864e5);
		const hours: number = Math.floor(ms % 864e5 / 36e5);
		const mins: number = Math.floor(ms % 864e5 % 36e5 / 6e4);
		const secs: number = Math.floor(ms % 864e5 % 36e5 % 6e4 / 1000);
		...
		difference.toSimplifiedString = () =>
			timeString.replace(/ays|ours|ins|ecs| /g, '').replace(/,/g, ' ')
				.trim();
